...
bool OFEncoder::begin(size_t sblen, size_t rblen) {
    ...
    of_sess_params_->nb_source_symbols = (uint32_t)sblen_;
    of_sess_params_->nb_repair_symbols = (uint32_t)rblen_;
...
void OFEncoder::commit() {
    ...
    for (size_t i = sblen_; i < sblen_ + rblen_;
         ++i) {
        if (OF_STATUS_OK
